...
public sealed class NobleRaritySortPatch : IPatchMethod
{
    ...
    {
        return [PatchTarget.Method<NCardGrid>("GetCardRarityComparisonValue")];
    }

    [HarmonyPrefix]
    private static bool Prefix(CardModel a, ref int __result)
    {
...
internal static class NoblePoolFilterRegistration
{
    ...
    public const string IconTexturePath = "res://Fgo/images/charui/character_icon_noble.png";
}
...
public sealed class NobleTitleOutlinePatch : IPatchMethod
{
    ...
    public static ModPatchTarget[] GetTargets()
    {
        return [PatchTarget.Method<NCard>("GetTitleLabelOutlineColor")];
    }

    private static bool Prefix(NCard __instance, ref Color __result)
    {
...
public sealed class NobleRarityLockPatch : IPatchMethod
{
    ...
    public static string Description =>
        "Disable rarity tickboxes when Noble pool filter is selected (like misc/ancients)";

    public static bool IsCritical => false;

    public static ModPatchTarget[] GetTargets()
    {
        return [PatchTarget.Method<NCardLibrary>("UpdateCardPoolFilter")];
    }
    ...
    private static void Postfix(NCardLibrary __instance, NCardPoolFilter filter)
    {
        ...
        // 如果不是 Noble filter 不做任何事
        if (filter.Name != "MOD_FILTER_SHARED_Noble" || !filter.IsSelected)
            return;
...
{
    // 反射获取 _poolFilters（不依赖 Harmony 字段注入，字段名带前导下划线，
    // ___poolFilters 会查不到，___poolFilters 又与记忆中"3 个下划线"约定冲突）
    ...
    public static string Description =>
        "Replace Noble pool filter predicate with c.Pool is NobleCardPool for robust filtering";

    public static bool IsCritical => false;

    public static ModPatchTarget[] GetTargets()
    {
        return [PatchTarget.Method<NCardLibrary>(nameof(NCardLibrary._Ready))];
    }
    ...
    {
        if (PoolFiltersField?.GetValue(__instance) is not IDictionary poolFilters)
            return;

        // 找到 Ritsu 创建的 Noble filter
        NCardPoolFilter? nobleFilter = null;
        foreach (var key in poolFilters.Keys)
            if (key is NCardPoolFilter f && f.Name == "MOD_FILTER_SHARED_Noble")
            {
                nobleFilter = f;
                break;
            }
        ...
        poolFilters[nobleFilter] = (Func<CardModel, bool>)(c => c.Pool is NobleCardPool);
    }
}